...
c3D.saveParaview(pts3D, os.getcwd(), "cuboid", n_eval_per_elem=20)
c2D = BSpline(c3D.getDegrees()[:-1], c3D.getKnots()[:-1])
pts2D = pts3D[..., -1]
c2D.saveParaview(pts2D, os.getcwd(), "squaroid", n_eval_per_elem=20)
c1D = BSpline(c2D.getDegrees()[:-1], c2D.getKnots()[:-1])
pts1D = pts2D[..., -1]
c1D.saveParaview(pts1D, os.getcwd(), "linoid", n_eval_per_elem=20)
# %%
