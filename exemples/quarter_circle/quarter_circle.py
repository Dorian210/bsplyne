...
ax.set_xticklabels([r'\$el_' + str(i) + r'\$' for i in range(len(el_borders) - 1)], minor=True)
ax.tick_params(which='minor', length=0)
ax.set_xlabel(r"\$\xi\$")
ax.set_yticks([0, 1])
plt.legend([r"\$N_" + str(i) + r"(\xi)\$" for i in range(c.bases[0].n + 1)], bbox_to_anchor=(1., 1.))
fig.tight_layout()
...
ax.plot(np.cos(np.pi/2*xi), np.sin(np.pi/2*xi), c='k', linewidth=1, linestyle="--", label=r"Exact quarter circle")
a = 0.35*np.pi
...
for i in range(locs.shape[1]):
    ax.annotate(r"\$el_" + str(i) + r"\$", locs[:, i]*1.15, fontsize=10, ha='center', va="center")
i = 2
init = pts[:, i].copy()
pts[:, i] *= 0.75
...
for i in range(c.bases[0].n + 1):
    ax.annotate(r"\$P_" + str(i) + r"\$", pts[:, i]*0.85, c=colors[i], fontsize=10, ha='center', va="center")
vals = c(pts, XI)
seg = np.concatenate((vals[:, None, :-1].T, vals[:, None, 1:].T), axis=1)
...
ax.annotate(
    ...
    xytext=(0.2, 0.),           # The position for the text (x, y)
