# %% Imports
from bsplyne import BSpline
import numpy as np
import matplotlib.pyplot as plt
import scipy.sparse as sps

# %% Fit a semicircle with a B-spline curve
def semicircle(xi):
    return np.cos(np.pi*xi), np.sin(np.pi*xi)

degrees = [2]
knots = [np.array([0, 0, 0, 1, 1, 1])]
spline = BSpline(degrees, knots)
spline.orderElevation(None, [1])
spline.knotInsertion(None, [2])

XI, dXI = spline.gauss_legendre_for_integration()
xi, = XI
dxi, = dXI

N = spline.DN(XI).tocsc()
A = N.T.multiply(dxi) @ N

x, y = semicircle(xi)
bx = N.T.multiply(dxi) @ x
by = N.T.multiply(dxi) @ y

ctrl_x = sps.linalg.spsolve(A, bx)
ctrl_y = sps.linalg.spsolve(A, by)

ctrl_pts = np.array([ctrl_x, ctrl_y], dtype='float')

ax = spline.plot(ctrl_pts, show=False)
ax.plot(*semicircle(np.linspace(0, 1, 100)), 'r-', lw=0.5, label='semicircle', zorder=100)
ax.legend()
plt.show()

# %% Save the B-spline curve to a file
spline.save('saved_shapes/semicircle.json', ctrl_pts)
# can also save as pickle file
spline.save('saved_shapes/semicircle.pkl', ctrl_pts)

# %% Define a square-to-disk mapping function to be fitted by a B-spline
def disk(xi, eta):
    """
    Map a square [0,1]^2 to the [-1,1]^2 disk.
    """
    xi = 2 * np.array(xi) - 1
    eta = 2 * np.array(eta) - 1
    r = np.maximum(np.abs(xi), np.abs(eta))
    theta = np.arctan2(eta, xi)
    x = r*np.cos(theta)
    y = r*np.sin(theta)
    return x, y

xi, eta = np.linspace(0, 1, 30), np.linspace(0, 1, 30)
XI, ETA = np.meshgrid(xi, eta, indexing='ij')
X, Y = disk(XI, ETA)
plt.plot(X, Y, 'k-', lw=0.5)
plt.plot(X.T, Y.T, 'k-', lw=0.5)
plt.gca().set_aspect('equal')

# %% Create a 2D B-spline surface and fit it to a disk using least squares
degrees = [2, 2]
knots = [np.array([0, 0, 0, 1, 1, 1]), np.array([0, 0, 0, 1, 1, 1])]
spline = BSpline(degrees, knots)
spline.knotInsertion(None, [2, 2])

# Discretize the parametric space and compute heavy weights for the border points
XI = spline.linspace(10)
xi, eta = np.meshgrid(*XI, indexing='ij')
border_mask = np.isclose(xi, 0) | np.isclose(xi, 1) | np.isclose(eta, 0) | np.isclose(eta, 1)
W = np.where(border_mask, 1e5, 1).ravel() # heavy weight on the border

# Compute the target disk coordinates
x_hat, y_hat = disk(xi, eta)

# Compute the LHS operators
N = spline.DN(XI).tocsc()
A = N.T.multiply(W) @ N

# Compute the RHS for the x and y coordinates problems
bx = N.T.multiply(W) @ x_hat.ravel()
by = N.T.multiply(W) @ y_hat.ravel()

# Solve for control points (weighted least squares fitting)
X = sps.linalg.spsolve(A, bx)
Y = sps.linalg.spsolve(A, by)

# Reshape and plot the fitted control net
disk_ctrl_pts = np.array([X, Y], dtype='float').reshape((2, *spline.getCtrlShape()))
spline.plot(disk_ctrl_pts)

# %% Save the B-spline surface to a file
spline.save('saved_shapes/disk.json', disk_ctrl_pts)
# can also save as pickle file
spline.save('saved_shapes/disk.pkl', disk_ctrl_pts)

# %% Let's do the same in 3D for a ball
def ball(xi, eta, zeta):
    """
    Map a cube [0,1]^3 to the [-1,1]^3 ball.
    """
    p = 50
    xi = 2*np.array(xi) - 1
    eta = 2*np.array(eta) - 1
    zeta = 2*np.array(zeta) - 1
    n_inf = np.maximum(np.maximum(np.abs(xi), np.abs(eta)), np.abs(zeta))
    npx = np.sqrt(xi**p + eta**2 + zeta**2)
    npy = np.sqrt(xi**2 + eta**p + zeta**2)
    npz = np.sqrt(xi**2 + eta**2 + zeta**p)
    xc = npx*np.tan(xi*np.arctan(1/np.where(npx==0, 1, npx)))
    yc = npy*np.tan(eta*np.arctan(1/np.where(npy==0, 1, npy)))
    zc = npz*np.tan(zeta*np.arctan(1/np.where(npz==0, 1, npz)))
    n = np.sqrt(xc**2 + yc**2 + zc**2)
    n = np.where(n==0, 1, n)
    x = n_inf*xc/n
    y = n_inf*yc/n
    z = n_inf*zc/n
    return x, y, z

n = 10
xi, eta, zeta = np.linspace(0, 1, n), np.linspace(0, 1, n), np.linspace(0, 1, n)
XI, ETA, ZETA = np.meshgrid(xi, eta, zeta, indexing='ij')
X, Y, Z = ball(XI, ETA, ZETA)

fig = plt.figure()
ax = fig.add_subplot(projection='3d')
for k in range(0, n):
    ax.plot_wireframe(X[:, :, k], Y[:, :, k], Z[:, :, k], color='k', linewidth=0.5)
    ax.plot_wireframe(X[:, k, :], Y[:, k, :], Z[:, k, :], color='k', linewidth=0.5)
    ax.plot_wireframe(X[k, :, :], Y[k, :, :], Z[k, :, :], color='k', linewidth=0.5)
ax.set_box_aspect((1, 1, 1))
plt.show()

# %% Create a 3D B-spline volume and fit it to a ball using least squares
degrees = [2, 2, 2]
knots = [np.array([0, 0, 0, 1, 1, 1]), np.array([0, 0, 0, 1, 1, 1]), np.array([0, 0, 0, 1, 1, 1])]
spline = BSpline(degrees, knots)
spline.knotInsertion(None, [1, 1, 1])

# Discretize the parametric space and compute heavy weights for the border points
XI = spline.linspace(10)
xi, eta, zeta = np.meshgrid(*XI, indexing='ij')
border = np.isclose(xi, 0) | np.isclose(xi, 1) | np.isclose(eta, 0) | np.isclose(eta, 1) | np.isclose(zeta, 0) | np.isclose(zeta, 1)
W = np.where(border, 1e5, 1).ravel()

# Compute the target ball coordinates
x_hat, y_hat, z_hat = ball(xi, eta, zeta)

# Compute the LHS operators
N = spline.DN(XI).tocsc()
A = N.T.multiply(W) @ N

# Compute the RHS for the x, y and z coordinates problems
bx = N.T.multiply(W) @ x_hat.ravel()
by = N.T.multiply(W) @ y_hat.ravel()
bz = N.T.multiply(W) @ z_hat.ravel()


# Solve for control points (weighted least squares fitting)
X = sps.linalg.spsolve(A, bx)
Y = sps.linalg.spsolve(A, by)
Z = sps.linalg.spsolve(A, bz)

# Reshape and plot the fitted B-spline
ball_ctrl_pts = np.array([X, Y, Z], dtype='float').reshape((3, *spline.getCtrlShape()))
spline.plot(ball_ctrl_pts)

# %% Save the B-spline volume to a file
spline.save('saved_shapes/ball.json', ball_ctrl_pts)
# can also save as pickle file
spline.save('saved_shapes/ball.pkl', ball_ctrl_pts)

# %%
